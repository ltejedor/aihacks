...
(async () => {
  ...
  const scored = loadJSON(cfg.paths.scored, null);
  ...
  const protectList = loadJSON(cfg.protectFile, []);
  ...
  const toPhone = (id) => {
    if (!id) return null;
    const raw = String(id);
    if (raw.includes('@lid')) return norm(identities.lidToPhone[raw] || identities.lidToPhone[raw.replace('@lid', '')] || raw);
    return norm(raw);
  };

  // ── GUARD: refuse on stale or mostly-unresolved data ──
  let newest = 0;
  for (const m of messages) newest = Math.max(newest, toMs(m.timestamp));
  ...
  for (const m of messages) if (toMs(m.timestamp) >= win) { const p = toPhone(m.authorId); if (p) recent.add(p); }
  for (const r of reactions) if (toMs(r.timestamp) >= win) { const p = toPhone(r.senderId); if (p) recent.add(p); }

  // ── protect set (bare digits + literal) ──
  const protect = new Set();
  for (const id of protectList) { const raw = String(id); protect.add(raw); const d = norm(raw); if (d) protect.add(d); }
  ...
  fs.mkdirSync(path.dirname(cfg.paths.candidates), { recursive: true });
  fs.writeFileSync(cfg.paths.candidates, JSON.stringify(out, null, 2));
  const totalProposable = Object.values(out.groups).reduce((a, g) => a + g.proposable.length, 0);
  console.log(JSON.stringify({ guard: out.guard, groups: Object.keys(out.groups).length, totalProposable, wrote: path.basename(cfg.paths.candidates) }));
