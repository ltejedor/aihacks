...
(async () => {
  ...
  try {
    ...
    let added = 0;
    for (const r of res.rows) { if (r.phone && r.name && !ledger[r.phone]) { ledger[r.phone] = r.name; added++; } } // grow-only
    fs.mkdirSync(path.dirname(cfg.paths.nameLedger), { recursive: true });
    fs.writeFileSync(cfg.paths.nameLedger, JSON.stringify(ledger, null, 2));

    console.log(JSON.stringify({ contacts: res.contacts, named: res.named, added, ledgerSize: Object.keys(ledger).length }));
  } finally {
