...
(async () => {
  ...
  let protect = [];
  try { protect = JSON.parse(fs.readFileSync(cfg.protectFile, 'utf8')); } catch (e) {}
  if (new Set(protect.map(String)).has(phone) || protect.map(String).some((p) => p.replace(/[^0-9]/g, '') === phone)) {
