...
Tending isn't only pruning — the higher-value move is **introducing** two members who should
know each other. Matching on a **shared domain or vision** beats matching on a generic skill: two
people both "into AI" is noise; two people independently building the same kind of thing, or
chasing the same outcome, is a real introduction. When you spot a match, make a small new group
for them (`tools/create-group.js`). The hard-won pattern:

1. Create the group **empty** (just the linked account). Seeding it with members up front throws
   "Lid is missing in chat table" on current WhatsApp Web and leaves an orphaned empty group.
2. Grab its **invite link**, then post the **intro** in the group so there's context the moment
   anyone joins.
3. **DM each person CONTEXT first, then the link as an opt-in** — one line on why you think
   they'd click, and the link offered, not forced. Never a bare link, never a silent force-add.

See `docs/CONNECTING.md` for the full flow and the reasoning behind it.

### Dossiers — what you learn about people

Accumulate what you learn about members as you go. Every finding — "co-founded X," "asked twice
about robotics," "operator vouched for them" — goes onto that person's **dossier** via `annotate`
(`store.annotate({ phone, note, name, by, protect })`), always with **provenance** (`by`: where
the fact came from — web, an admin reply, your own read) and an optional `protect` flag when the
finding means they should never be cut. Dossiers are **community knowledge**: facts about the
people in the garden, reused by research, reports, and introductions. They are **distinct from
`memory/`**, which is your own self-knowledge — how the operator works, what you got wrong, your
own evolving voice. Don't conflate the two: dossiers are about *them*, memory is about *you*.
...
- **Confirm before each removal; batch the approved set in one session.** Don't mass-remove
  unilaterally.
- **Explain before you send a link or make an ask.** Give one line of context and offer the
  link/ask as an opt-in — a bare link reads as spam, and a cold ask reads as a demand.
